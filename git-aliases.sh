...
# Branch
alias delbr='! f() { git branch -D $1; }; f'
alias delbr1='! f() { git branch | grep -v "master" | xargs git branch -D; }; f'
alias delbr2='! f() { git branch | grep $1 | xargs git branch -D; }; f'
# checkout remote branch and track
# $1 remote branch name
alias newbr='! f() { git checkout --track -b $1; }; f'
# checkout remote branch which has the same name with local branch
alias newbr1='! f() { git checkout --track -b $1 origin/$1; }; f'
